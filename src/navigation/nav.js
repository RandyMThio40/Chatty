    ...
    return(
        <nav className="Nav-bar">
            <Link to={`/${currentUser.uid}/home`} >
                <h1 className="Nav-header">Chatty</h1>
            </Link>
            <div ref={settingRef} className='user-setting-container'>
                <div className="user-setting-wrapper" onClick={toggleActive}>
                    <button className='hidden-butt' onClick={toggleActive} ></button>
                    ...
                    <h3 className="username">{displayName || currentUser?.displayName || currentUser?.email}</h3>
                </div>
